...
CREATE TABLE duplicates.tmp_com_nordstrom_order_item_attrs_id -- todo: replace placeholder name
  DISTKEY (root_id)
  SORTKEY (root_id)
AS (SELECT root_id FROM (SELECT root_id, COUNT(*) AS count FROM public.com_nordstrom_order_item_attrs GROUP BY 1) WHERE count > 1); -- todo: replace placeholder name
...
AS (
  ...
  FROM public.com_nordstrom_order_item_attrs -- todo: replace placeholder name
...
BEGIN;
  ...
  DELETE FROM public.com_nordstrom_order_item_attrs WHERE root_id IN (SELECT root_id FROM duplicates.tmp_com_nordstrom_order_item_attrs_id); -- todo: replace placeholder name
  INSERT INTO public.com_nordstrom_order_item_attrs (SELECT * FROM duplicates.tmp_com_nordstrom_order_item_attrs); -- todo: replace placeholder name
...
BEGIN;
  ...
  INSERT INTO duplicates.com_nordstrom_order_item_attrs (SELECT * FROM public.com_nordstrom_order_item_attrs WHERE root_id IN (SELECT root_id FROM duplicates.tmp_com_nordstrom_order_item_attrs_id_remaining)); -- todo: replace placeholder name
  DELETE FROM public.com_nordstrom_order_item_attrs WHERE root_id IN (SELECT root_id FROM duplicates.tmp_com_nordstrom_order_item_attrs_id_remaining); -- todo: replace placeholder name
