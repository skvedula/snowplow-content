...
DROP TABLE IF EXISTS duplicates.tmp_events2;
...
--   DISTKEY (event_id)
--   SORTKEY (event_id)
-- AS (SELECT event_id, event_fingerprint FROM (SELECT event_id, event_fingerprint, COUNT(*) AS count FROM public.events WHERE event_fingerprint IS NOT NULL GROUP BY 1,2) WHERE count > 1);
...
--CREATE TABLE duplicates.tmp_events_id_remaining
  --DISTKEY (event_id)
  --SORTKEY (event_id)
--AS (SELECT event_id FROM (SELECT event_id, COUNT(*) AS count FROM public.events WHERE event_fingerprint IS NOT NULL GROUP BY 1) WHERE count > 1);

--BEGIN;

  --INSERT INTO duplicates.events (SELECT * FROM public.events WHERE event_fingerprint IS NOT NULL AND event_id IN (SELECT event_id FROM duplicates.tmp_events_id_remaining));
  --DELETE FROM public.events WHERE event_fingerprint IS NOT NULL AND event_id IN (SELECT event_id FROM duplicates.tmp_events_id_remaining);

--COMMIT;

-- (d) final de-dupe to create unique event_id by keeping only the earliest collector timestamp
...
CREATE TABLE duplicates.tmp_events2
 DISTKEY (event_id)
 SORTKEY (event_id)
AS (

 SELECT *, ROW_NUMBER() OVER (PARTITION BY event_id ORDER BY collector_tstamp) as event_number
 FROM public.events
